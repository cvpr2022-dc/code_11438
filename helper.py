...
class logger:
    ...
    def conditional_save_img_comparison_new(self, mode, i, ele, rgb_conf_final, semantic_conf_final, d_conf,rgb_depth_final,semantic_depth_final, d_depth, coarse_depth,pred, epoch, predrgb=None, predg=None, extra=None, extra2=None, extrargb=None):
        # save 8 images for visualization
        if mode == 'val' or mode == 'eval':
            skip = 100
            if i == 0:
                self.img_merge1, self.img_merge2 = vis_utils.merge_into_row(ele,  rgb_conf_final, semantic_conf_final, d_conf,rgb_depth_final,semantic_depth_final, d_depth, coarse_depth, pred, predrgb, predg, extra, extra2, extrargb)
                #self.img_merge1 = vis_utils.merge_into_row(ele, rgb_conf_1, semantic_conf_1, rgb_conf_2, semantic_conf_2, rgb_conf_final, semantic_conf_final, d_conf, pred, predrgb, predg, extra, extra2, extrargb)            
            elif i % skip == 0 and i < 8 * skip:
            
                row1,row2 = vis_utils.merge_into_row(ele, rgb_conf_final, semantic_conf_final, d_conf,rgb_depth_final,semantic_depth_final, d_depth, coarse_depth, pred, predrgb, predg, extra, extra2, extrargb)
                #row1 = vis_utils.merge_into_row(ele, rgb_conf_1, semantic_conf_1, rgb_conf_2, semantic_conf_2, rgb_conf_final, semantic_conf_final, d_conf, pred, predrgb, predg, extra, extra2, extrargb)
                self.img_merge1 = vis_utils.add_row(self.img_merge1, row1)
                self.img_merge2 = vis_utils.add_row(self.img_merge2, row2)
            elif i == 8 * skip:
                filename = self._get_img_comparison_name(mode, epoch)
                vis_utils.save_image(self.img_merge1, filename + ".png")
                vis_utils.save_image_gray(self.img_merge2, filename + "conf.png")
    ...
    def save_img_comparison_as_best(self, mode, epoch):
        if mode == 'val':
            ...
            vis_utils.save_image(self.img_merge1, filename)
...
def backup_source_code(backup_directory):
    ...
    shutil.copytree("/netscratch/afzal/Depth-Completion_Experiments/input=rgbd.criterion=l2.lr=0.025298.bs=16.wd=1e-06.jitter=0.1.time=2021-10-31@10-45/code_backup/", backup_directory, ignore=ignore_hidden)
